...
// ---------------------------------------------------------------------------
async function crmFetch(path, method, body) {
    const baseUrl = process.env.CRM_BASE_URL;
    const secret = process.env.DONNA_API_SECRET;
    if (!baseUrl)
        throw new Error("CRM_BASE_URL no está configurado en .env");
    if (!secret)
        throw new Error("DONNA_API_SECRET no está configurado en .env");
...
export async function executeCrmSearchLead(args) {
    try {
        const data = await crmFetch(`/api/bot/leads?q=${encodeURIComponent(args.query)}`, "GET");
        return JSON.stringify(data, null, 2);
    }
    catch (err) {
        return `Error buscando lead: ${err.message}`;
    }
}
// ---------------------------------------------------------------------------
...
export async function executeCrmCreateLead(args) {
    try {
        const payload = {
            name: args.name,
        ...
        const data = await crmFetch("/api/bot/leads", "POST", payload);
        const id = data?.data?.id ?? "?";
        return `✅ Lead "${args.name}" creado exitosamente en el CRM con ID: ${id}.`;
    }
    catch (err) {
        return `Error creando lead: ${err.message}`;
    }
}
// ---------------------------------------------------------------------------
...
export async function executeCrmCreateQuotation(args) {
    try {
        const payload = {
            ...args,
            status: "draft",
        ...
        const data = await crmFetch("/api/bot/quotations", "POST", payload);
        const id = data?.data?.id ?? "?";
        ...
        return `Error creando cotización: ${err.message}`;
    }
}
// ---------------------------------------------------------------------------
...
export async function executeCrmCreateContract(args) {
    try {
        const payload = {
            ...args,
            status: "draft",
        ...
        const data = await crmFetch("/api/bot/contracts", "POST", payload);
        const id = data?.data?.id ?? "?";
        ...
        return `Error creando contrato: ${err.message}`;
    }
}
// ---------------------------------------------------------------------------
...
export async function executeCrmUpdateLeadStatus(args) {
    try {
        ...
        return `Error actualizando estado del lead: ${err.message}`;
    }
}
// ---------------------------------------------------------------------------
...
export async function executeGetCompanyKnowledge() {
    try {
        const filePath = path.resolve("./src/data/knowledge.md");
        return fs.readFileSync(filePath, "utf-8");
    }
    catch (err) {
        return `Error leyendo archivo de conocimiento: ${err.message}`;
    }
}
// ---------------------------------------------------------------------------
...
export async function executeUpdateKnowledgeBase(args) {
    try {
        const filePath = path.resolve("./src/data/knowledge.md");
        ...
        if (existing.includes(marker)) {
            updated = existing.replace("*(Este espacio se actualiza cuando César te corrige o te enseña algo nuevo)*", `*(Este espacio se actualiza cuando César te corrige o te enseña algo nuevo)*${newEntry}`);
        ...
        return `Error guardando aprendizaje: ${err.message}`;
    }
}
// ---------------------------------------------------------------------------
...
export const crmSendDocumentSchema = z.object({
    phone: z.string().describe("Número de teléfono del destinatario con código de país, sin '+'. Ejemplo: 593966410409"),
    ...
    messageText: z.string().optional().describe("Mensaje de WhatsApp que acompañará el PDF. Si no se provee, el CRM usará uno genérico"),
...
export const crmSendDocumentDefinition = {
    ...
    function: {
        ...
        description: "Envía un documento (propuesta o contrato) redactado en Markdown al CRM. El CRM lo convierte en PDF y lo envía por WhatsApp al cliente en un solo paso. Úsalo cuando César confirme que quiere generar el PDF y enviarlo.",
        parameters: {
            type: "object",
            properties: {
                phone: { type: "string", description: "Número del destinatario con código de país sin '+' (ej: 593966410409)" },
                ...
                messageText: { type: "string", description: "Mensaje de WhatsApp que acompañará el PDF" },
...
export async function executeCrmSendDocument(args) {
    try {
        const payload = {
            phone: args.phone,
        ...
        const data = await crmFetch("/api/bot/send-document", "POST", payload);
        const msgId = data?.data?.whatsappMessageId ?? "?";
        const fileName = data?.data?.fileName ?? args.title ?? "documento";
        return `✅ PDF "${fileName}" generado y enviado por WhatsApp exitosamente. ID del mensaje: ${msgId}.`;
    }
    catch (err) {
        return `❌ Error enviando documento: ${err.message}`;
    }
}
