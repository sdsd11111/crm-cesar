import { createChatCompletion, ChatMessage } from "./llm.js";
...
export async function runAgentLoop(userId: string, userMessage: string): Promise<string> {
  ...
  const conversation: ChatMessage[] = historyRows.map(row => ({
    role: row.role as "system" | "user" | "assistant",
    content: row.content,
  }));

  // Construct messages for LLM
  let messages: ChatMessage[] = [SYSTEM_PROMPT, ...conversation];
  let finalResponse = "";
  ...
  for (let step = 0; step < MAX_ITERATIONS; step++) {
    ...
    if (message.tool_calls && message.tool_calls.length > 0) {
      ...
      if (forcedBreak) continue; // Go to next iteration so LLM can output text
    } else {
