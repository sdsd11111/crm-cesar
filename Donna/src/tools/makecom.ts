...
const SCENARIOS: Record<string, { label: string; envKey: string }> = {
    "publicar_activaqr": {
...
type TriggerMakeArgs = z.infer<typeof triggerMakeSchema>;
...
export async function executeTriggerMake(args: TriggerMakeArgs): Promise<string> {
    ...
    try {
        ...
        return `❌ Error activando Make.com: ${(err as Error).message}`;
    }
}
