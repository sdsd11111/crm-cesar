...
function repeat(fn, every24h = true) {
    fn();
    if (every24h) {
...
async function sendDailyBriefing() {
    ...
    try {
        ...
        if (events.length === 0) {
            await bot.api.sendMessage(primaryUserId, "🌅 *Buenos días, César!* Tu día está libre, sin citas registradas en el calendario. ¡Perfecto para cerrar ventas desde la calle! 💪", { parse_mode: "Markdown" });
            return;
        }
