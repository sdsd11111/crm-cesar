...
type WhatsappSendArgs = z.infer<typeof whatsappSendSchema>;
...
export async function executeWhatsappSend(args: WhatsappSendArgs): Promise<string> {
    ...
    try {
        ...
        return `❌ Error enviando WhatsApp: ${(err as Error).message}`;
    }
}
