...
export default function ChatCenterPage() {
    ...
    const handleSendMessage = async () => {
        const currentText = selectedChat ? (chatDrafts[selectedChat.id] || '') : '';
        console.log('🚀 Sending message attempt:', { phone: selectedChat?.phone, text: currentText, media: pendingMedia });

        if (!currentText.trim() && !pendingMedia) {
        ...
        try {
            const res = await fetch('/api/whatsapp/send', {
                ...
                if(data.success) {
                    ...
                    setChatDrafts(prev => ({ ...prev, [selectedChat.id]: '' }));
                                ...
                                <div className="max-w-3xl mx-auto flex flex-col gap-2">
                                    ...
                                    <div className="flex items-end gap-3">
                                        <div className="flex-1 relative group">
                                            ...
                                            <Textarea
                                                value={selectedChat ? (chatDrafts[selectedChat.id] || '') : ''}
                                                onChange={(e) => {
                                                    if (selectedChat) {
                                                        const val = e.target.value;
                                                        setChatDrafts(prev => ({ ...prev, [selectedChat.id]: val }));
                                                    }
                                                }}
                                        ...
                                        <Button
                                            ...
                                            disabled={isSending || isUploading || (!selectedChat || (!chatDrafts[selectedChat.id]?.trim() && !pendingMedia))}
