...
export default function LeadDetailPage() {
    ...
    const handleGenerateBookingProposal = async () => {
        setIsGeneratingProposal(true);
        try {
            const rawName = lead.contactName || '';
            const hotel = lead.businessName || '((HOTEL))';
            const city = lead.city || 'su zona';

            let template = `Estimada ${rawName || '((NOMBRE))'}, muy buen día.

Un gusto saludarte.
Estuve revisando la presencia digital de ${hotel} y, como te comenté, tienen una muy buena propuesta: hotel nuevo, bien presentado y con excelentes valoraciones, lo cual es una gran ventaja.
                            ...
                            <div className="lg:col-span-1">
                                <Card className="border-none shadow-sm bg-muted/20">
                                    ...
                                    <CardContent className="space-y-4 text-sm">
                                        ...
                                        {/* Quick Interaction Button relocated to sidebar */}
                                        <Dialog open={isQuickInteractionOpen} onOpenChange={setIsQuickInteractionOpen}>
                                            <DialogTrigger asChild>
                                                <Button
                                                    variant="outline"
                                                    className="w-full py-8 mt-2 border-dashed border-2 hover:border-primary hover:bg-primary/5 transition-all group flex flex-col gap-1 items-center justify-center rounded-xl"
                                                >
                                                    <div className="p-2 bg-muted rounded-full group-hover:bg-primary/10 transition-colors">
                                                        <Plus className="h-6 w-6 text-muted-foreground group-hover:text-primary" />
                                                    </div>
                                                    <span className="text-[10px] font-bold text-muted-foreground group-hover:text-primary uppercase tracking-wider text-center">Registrar Nueva Interacción</span>
                                                </Button>
                                            </DialogTrigger>
                                        ...
                                        <div className="pt-4 border-t mt-4 space-y-2">
                                            <p className="text-[10px] font-bold text-muted-foreground uppercase px-1">Propuestas Especiales</p>
                                            <Button
                                                variant="outline"
                                                ...
                                                onClick={handleGenerateBookingProposal}
