...
type GmailSearchArgs = z.infer<typeof gmailSearchSchema>;
...
export async function executeGmailSearch(args: GmailSearchArgs): Promise<string> {
    try {
        ...
        return `Error en gmail_search: ${(error as Error).message}`;
    }
}
...
type GmailSendArgs = z.infer<typeof gmailSendSchema>;
...
export async function executeGmailSend(args: GmailSendArgs): Promise<string> {
    try {
        const auth = await authorize();
        const gmail = google.gmail({ version: "v1", auth });
        ...
        return `Error enviando correo: ${(error as Error).message}`;
    }
}
...
type CalendarListArgs = z.infer<typeof calendarListSchema>;
...
export async function executeCalendarList(args: CalendarListArgs): Promise<string> {
    try {
        const auth = await authorize();
        ...
        return `Error listando eventos: ${(error as Error).message}`;
    }
}
...
type CalendarCreateArgs = z.infer<typeof calendarCreateSchema>;
...
export async function executeCalendarCreate(args: CalendarCreateArgs): Promise<string> {
    try {
        const auth = await authorize();
        const calendar = google.calendar({ version: "v3", auth });
        ...
        return `Error creando evento: ${(error as Error).message}`;
    }
}
